...
// サイコロの面コンポーネント（チンチロ3Dと同じ実装）
export function DiceFace({ position, rotation, number, size }: DiceFaceProps) {
  ...
  // 各面のロゴ配置パターン（チンチロ3Dと同じ）
  const getLogoPositions = (faceNumber: number) => {
    switch (faceNumber) {
      case 1:
